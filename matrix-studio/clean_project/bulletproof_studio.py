"""
ATLANTIPLEX STUDIO - BULLETPROOF VERSION
Guaranteed to work and stay running
"""
...
app = Flask(__name__)
app.secret_key = 'bulletproof_key_2024'
...
def get_port():
    """Get a working port - bulletproof method"""
...
def home():
    if 'username' in session:
        return render_template_string(BASIC_TEMPLATE, port=port)
    else:
...
def login():
    ...
    if not username or not password:
        return render_template_string(BASIC_TEMPLATE, error="Credentials required", port=port)
    ...
    try:
        ...
        return render_template_string(BASIC_TEMPLATE, error="Invalid credentials", port=port)
        
    except Exception as e:
        print(f"✗ Login error: {e}")
        return render_template_string(BASIC_TEMPLATE, error="System error", port=port)
...
def status():
    ...
    <body style="background: #000; color: #0f0; font-family: monospace; padding: 20px;">
        <h1>✓ ATLANTIPLEX STUDIO STATUS</h1>
        <p>Server: RUNNING</p>
        <p>Port: {port}</p>
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("ATLANTIPLEX STUDIO - BULLETPROOF VERSION")
    print("=" * 60)
