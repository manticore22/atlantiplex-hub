...
def test_guest_management_core():
    ...
    try:
        ...
        print("[PASS] Guest management module imported successfully")
    except ImportError as e:
        print(f"[FAIL] CRITICAL: Cannot import guest_management: {e}")
        return False
    ...
    try:
        ...
        print("[PASS] Guest Manager initialized correctly")
    except Exception as e:
        print(f"[FAIL] Failed to initialize Guest Manager: {e}")
        return False
    ...
    try:
        ...
        print(f"[PASS] Created {len(test_guests)} guest invitations")
    except Exception as e:
        print(f"[FAIL] Failed to create guest invitations: {e}")
        return False
    ...
    try:
        ...
        print(f"[PASS] {len(joined_guests)} guests joined successfully")
    except Exception as e:
        print(f"[FAIL] Failed guest join test: {e}")
        return False
    ...
    try:
        ...
        print("[PASS] Slot management working correctly")
    except Exception as e:
        print(f"[FAIL] Failed slot management test: {e}")
        return False
    ...
    try:
        if active_guests:
            ...
            if test_guest:
                ...
                print("[PASS] Camera state ON working")
                
                manager.set_guest_media_state(test_guest.id, camera=MediaState.OFF)
                assert test_guest.device.camera_enabled == False
                print("[PASS] Camera state OFF working")
    ...
    try:
        if active_guests:
            ...
            print("[PASS] Device configuration working")
    except Exception as e:
        print(f"[FAIL] Failed device configuration test: {e}")
        return False
    ...
    try:
        ...
        print("[PASS] Studio status reporting working")
    except Exception as e:
        print(f"[FAIL] Failed studio status test: {e}")
        return False
    ...
    try:
        ...
        print(f"[PASS] Waiting room system working - {len(manager.waiting_room)} guests in queue")
    except Exception as e:
        print(f"[FAIL] Failed waiting room test: {e}")
        return False
    ...
    try:
        ...
        print(f"[PASS] Export successful - {export_data['total_guests']} total guests")
    except Exception as e:
        print(f"[FAIL] Failed export test: {e}")
        return False
    ...
    try:
        ...
        print("[PASS] Invalid guest ID handling working")
    except Exception as e:
        print(f"[FAIL] Failed security test: {e}")
        return False
...
def test_matrix_studio_api():
    ...
    try:
        sys.path.append('matrix-studio')
        from matrix_studio_final import app
        print("[PASS] Matrix Studio app imported successfully")
        
        # Test that the Flask app is configured
        assert app is not None
        print("[PASS] Flask app is configured")
        ...
        print(f"[PASS] Required API routes found ({len(required_routes)} routes)")
        print(f"[PASS] Total routes available: {len(routes)}")
        
        return True
        
    except ImportError as e:
        print(f"[FAIL] Cannot import Matrix Studio: {e}")
        return False
    except Exception as e:
...
def test_guest_html_interface():
    ...
    if not os.path.exists(guest_html_path):
        print(f"[FAIL] Guest HTML file not found: {guest_html_path}")
        return False
    ...
    try:
        ...
        print("[PASS] Guest HTML interface contains all required elements")
        print("[PASS] Guest interface is properly structured")
        
        return True
        
    except Exception as e:
        print(f"[FAIL] Failed to test guest HTML interface: {e}")
        return False
...
def main():
    ...
    if failed_suites == 0:
        ...
        print(f"\n{failed_suites} test suite(s) failed. Review the issues above.")
